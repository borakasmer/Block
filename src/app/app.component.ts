...
export class AppComponent {
  ...
  AddColumunOrderedSort(isAddNewColumn: boolean) {
    ...
    if (this.selectedSortType == 9) {
      ...
      if (!this.isAddRow)
        this.Container = this.sortService.AddColumnOrderedSortLeftToRight(this.rowCount, Number(currentList.length / this.rowCount) + (isAddNewColumn ? 1 : 0), currentList, this.isIgnoreGaps, this.selectedCounterType, this.columnCount)
      //Önceden satır eklenmiş ise buna kolon ekleyebiliriz. "this.Container.length" yeni eklenmiş dahil toplam satır sayısıdır. "this.Container[0].length" yeni eklenmiş dahil toplam kolon sayısıdır.
        ...
        this.Container = this.sortService.AddColumnOrderedSortLeftToRight(this.Container.length, this.Container[0].length + (isAddNewColumn ? 1 : 0), currentList, this.isIgnoreGaps, this.selectedCounterType, this.Container[0].length)
    }
    else if (this.selectedSortType == 10) {
      if (!this.isAddRow)
        this.Container = this.sortService.AddColumnOrderedSortRightToLeft(this.rowCount, Number(currentList.length / this.rowCount) + (isAddNewColumn ? 1 : 0), currentList, this.isIgnoreGaps, this.selectedCounterType, this.columnCount)
      else
        this.Container = this.sortService.AddColumnOrderedSortRightToLeft(this.Container.length, this.Container[0].length + (isAddNewColumn ? 1 : 0), currentList, this.isIgnoreGaps, this.selectedCounterType, this.Container[0].length)
    }
  }
}